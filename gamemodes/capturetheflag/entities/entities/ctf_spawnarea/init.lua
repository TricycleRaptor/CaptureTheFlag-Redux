...
function ENT:GetSpawnPos()
	...
	if self.Entity.SpawnIndex > 0 then
		local angle = (self.Entity.SpawnIndex - 1) / (team.NumPlayers(self.Entity:GetNWInt("Team")) - 1) * math.pi * 2
		dir = self.Entity:GetForward() * math.sin(angle) + self.Entity:GetRight() * math.cos(angle)
	end
	self.Entity.SpawnIndex = (self.Entity.SpawnIndex + 1) % team.NumPlayers(self.Entity:GetNWInt("Team"))
